...
`go-ruby-actionpack/actionpack` reimplements Ruby's `actionpack` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-actionpack/actionpack` directly, with no Ruby runtime;
...
Because `actionpack` is pure Go with cgo disabled, it also compiles to **WebAssembly**
